...
# 25. Function to install and run restart scheduler
install_restart_scheduler() {
  dialog --title "Install Restart Scheduler" --yesno "This will install and run the restart scheduler script. Proceed?" 10 60
  response=$?
  if [ $response -eq 0 ]; then
    sudo bash -c '
      set -e
    ...
    dialog --msgbox "Restart scheduler installed and started successfully!" 10 60
  else
    dialog --msgbox "Installation canceled." 10 40
  fi
}
...
while true; do
  choice=$(dialog --clear --backtitle "FreeIRAN v.1.4.0 - Main Menu" --title "Main Menu" --menu "Choose an option:" 30 60 25 \
    ...
    22 "Reboot System" \
    23 "Exit Script" \
    24 "Install Restart Scheduler" 3>&1 1>&2 2>&3)
